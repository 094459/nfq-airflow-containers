...
class AirflowCdkStackMWAA(Stack):
    ...
    def __init__(self, scope: Construct, id: str, vpc, airflow_props,  **kwargs) -> None:
        ...
        s3_tags = {
            'env': f"{airflow_props['airflow_env']}",
        ...
        dags_bucket = s3.Bucket(
            self,
            "mwaa-dags",
            bucket_name=f"{airflow_props['dagslocation'].lower()}",
            versioned=True,
        ...
        mwaa_policy_document = iam.PolicyDocument(
            statements=[
                iam.PolicyStatement(
                    ...
                    resources=[f"arn:aws:airflow:{self.region}:{self.account}:environment/{airflow_props['airflow_env']}"],
                ),
                iam.PolicyStatement(
                    actions=[
                        "s3:ListAllMyBuckets"
                ...
                iam.PolicyStatement(
                    ...
                    effect=iam.Effect.ALLOW,
                    resources=[f"arn:aws:logs:{self.region}:{self.account}:log-group:airflow-{airflow_props['airflow_env']}-*"],
                ),
                iam.PolicyStatement(
                    actions=[
                        "logs:DescribeLogGroups"
        ...
        key = kms.Key(
            self,
            f"{airflow_props['airflow_env']}{key_suffix}",
            enable_key_rotation=True,
            policy=kms_mwaa_policy_document
        )

        key.add_alias(f"alias/{airflow_props['airflow_env']}{key_suffix}")
        ...
        managed_airflow = mwaa.CfnEnvironment(
            ...
            name=f"{airflow_props['airflow_env']}",
